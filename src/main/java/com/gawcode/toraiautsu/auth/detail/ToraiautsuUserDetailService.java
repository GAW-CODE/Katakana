package com.gawcode.toraiautsu.auth.detail;
...
public class ToraiautsuUserDetailService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String s) throws UsernameNotFoundException {
        return (EMAIL_VALIDATOR.isValid(s, null) ? this.teamMemberService.find(s.toLowerCase(Locale.ENGLISH)) : this.teamMemberService.findByUsername(s)).map(this::createSpringSecurityUser).orElseThrow(() -> new UsernameNotFoundException("The team member with username/email " + s + " does not exist"));
    }
